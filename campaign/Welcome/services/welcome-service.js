angular.module('WelcomeApp')
    .factory('$WelcomeService', function ($http, $q) {
        ...
        function getUserLogItem(userName){
            return $http.get(_spPageContextInfo.webAbsoluteUrl.substring(0, _spPageContextInfo.webAbsoluteUrl.lastIndexOf('/')) + '/_api/web/lists/getbytitle(\'UsersLog\')/items?'
        ...
        function getAvatarsItems(filter){
            ...
            return $http.get(_spPageContextInfo.webAbsoluteUrl.substring(0, _spPageContextInfo.webAbsoluteUrl.lastIndexOf('/')) + '/_api/web/lists/getbytitle(\'Avatars\')/items?'+filter)
                .then(function(res){
                    return res.data.value;
                });
        }
        ...
        function createUserLogItem(data){
            return $http({
                method: "POST",
                url: _spPageContextInfo.webAbsoluteUrl.substring(0, _spPageContextInfo.webAbsoluteUrl.lastIndexOf('/')) + '/_api/web/lists/getbytitle(\'UsersLog\')/items?$select=*',
                data: data,
                headers: {
                    "Content-Type" : "application/json;odata=verbose",
                    "Accept": "application/json;odata=verbose",
        ...
        function updateUserLogItem(item){
            return new Promise(function(resolve, reject){
                var clientContext = new SP.ClientContext(_spPageContextInfo.webAbsoluteUrl.substring(0, _spPageContextInfo.webAbsoluteUrl.lastIndexOf('/')));
